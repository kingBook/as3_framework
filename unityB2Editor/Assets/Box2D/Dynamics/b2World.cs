...
public class b2World
{
	...
	*/
	 ...
	 */
	public void SetBroadPhase(IBroadPhase broadPhase){
	...
	*/
	 ...
	 */
	public b2Controller AddController(b2Controller c)
	{
	...
	public void RemoveController(b2Controller c)
	{
	...
	public b2Controller CreateController(b2Controller controller)
	{
		if (controller.m_world != this) {
			Debug.LogError ("Controller can only be a member of one world");
		}
	...
	public void DestroyController(b2Controller controller)
	{
	...
	*/
	...
	private static b2TimeStep s_timestep2 = new b2TimeStep();
	/**
	...
	*/
	...
	static private b2Transform s_xf = new b2Transform();
	/**
	...
	public void DrawDebugData(){
		...
		b2Vec2[] vs = new b2Vec2[]{new b2Vec2(), new b2Vec2(), new b2Vec2(), new b2Vec2()};
		
		// Store color here and reuse, to reduce allocations
		b2Color color = new b2Color(0.0f, 0.0f, 0.0f);
			...
			for (b = m_bodyList; b!=null; b = b.m_next)
			{
				...
				for (f = b.GetFixtureList(); f!=null; f = f.m_next)
				{
					...
					if (b.IsActive() == false)
					{
						...
						//DrawShape(s, xf, color);//edit by kingBook 2017-2-21
					}
					else if (b.GetType() == b2Body.b2_staticBody)
					{
						color.Set(0.5f, 0.9f, 0.5f);
		...
		if ((flags & b2DebugDraw.e_pairBit)>0)
		{
			...
			//edit by 2015/9/18 13:55 by kingBook
			//for (var contact:b2Contact = m_contactManager.m_contactList; contact; contact = contact.GetNext())
			for (b2Contact contact = m_contactList; contact!=null; contact = contact.GetNext())
			{
		...
		if ((flags & b2DebugDraw.e_aabbBit)>0)
		{
			...
			vs = new b2Vec2[]{new b2Vec2(),new b2Vec2(),new b2Vec2(),new b2Vec2()};
	 ...
	 */
	public void QueryAABB(b2WorldQueryCallback callback, b2AABB aabb)
	{
		IBroadPhase broadPhase = m_contactManager.m_broadPhase;
		BroadPhaseQueryCallback WorldQueryWrapper = delegate(object proxy){
			return callback((b2Fixture)broadPhase.GetUserData(proxy));
		};
	 ...
	 * @asonly
	 */
	public void QueryShape(b2WorldQueryCallback callback, b2Shape shape, b2Transform transform=null)
	{
		if (transform == null)
		{
	 ...
	 * @asonly
	 */
	public void QueryPoint(b2WorldQueryCallback callback, b2Vec2 p)
	{
		IBroadPhase broadPhase = m_contactManager.m_broadPhase;
		BroadPhaseQueryCallback WorldQueryWrapper=delegate(object proxy){
			b2Fixture fixture = broadPhase.GetUserData(proxy) as b2Fixture;
	 ...
	 * By returning 0, you immediately terminate.立即停止Raycast的查找
	 * By returning 1, you continue wiht the original ray.Raycast持续查找，直到达到线段的终点
	 * By returning the current fraction, you proceed to find the closest point.查找最近的碰撞刚体
	 * @param point1 the ray starting point
	 * @param point2 the ray ending point
	 */
	public void RayCast(b2WorldRayCastCallback callback, b2Vec2 point1, b2Vec2 point2)
	{
		IBroadPhase broadPhase = m_contactManager.m_broadPhase;
		b2RayCastOutput output = new b2RayCastOutput();
		BroadPhaseRayCastCallback RayCastWrapper=delegate(b2RayCastInput input, object proxy){
			object userData = broadPhase.GetUserData(proxy);
			...
			if (hit)
			{
				...
				b2Vec2 point = new b2Vec2(
					(1.0f - fraction) * point1.x + fraction * point2.x,
					(1.0f - fraction) * point1.y + fraction * point2.y);
				return callback(fixture, point, output.normal, fraction);
			}
			return input.maxFraction;
		};
		b2RayCastInput input2 = new b2RayCastInput(point1, point2);
		broadPhase.RayCast(RayCastWrapper, input2);
	}
	
	public b2Fixture RayCastOne(b2Vec2 point1, b2Vec2 point2)
	{
		b2Fixture result=null;
		b2WorldRayCastCallback RayCastOneWrapper=delegate(b2Fixture fixture, b2Vec2 point, b2Vec2 normal, float fraction){
			result = fixture;
			return fraction;
		};
	...
	public List<b2Fixture> RayCastAll(b2Vec2 point1, b2Vec2 point2)
	{
		List<b2Fixture> result = new List<b2Fixture>();
		b2WorldRayCastCallback RayCastAllWrapper=delegate(b2Fixture fixture, b2Vec2 point, b2Vec2 normal, float fraction){
			result.Add(fixture);
			return 1;
		};
	...
	*/
	...
	private Dictionary<int,b2Body> s_stack = new Dictionary<int,b2Body>();
	public void Solve(b2TimeStep step){
		...
		Dictionary<int,b2Body> stack = s_stack;
	...
	private static b2Sweep s_backupA = new b2Sweep();
	private static b2Sweep s_backupB = new b2Sweep();
	private static b2TimeStep s_timestep = new b2TimeStep();
	private static Dictionary<int,b2Body> s_queue = new Dictionary<int,b2Body>();
	...
	public void SolveTOI(b2TimeStep step){		
		...
		Dictionary<int,b2Body> queue = s_queue;
		...
		for (;;)
		{
			...
			for (c = m_contactList; c!=null; c = c.m_next)
			{
				...
				else
				{
					...
					b2Settings.b2Assert(0.0f <= toi && toi <= 1.0f);
				...
				if (float.MinValue < toi && toi < minTOI)
				{
			...
			if (minContact == null || 1.0f - 100.0f * float.MinValue < minTOI)
			{
	...
	static private b2Color s_jointColor = new b2Color(0.5f, 0.8f, 0.8f);
	//
	public void DrawJoint(b2Joint joint){
		...
		else if(joint.m_type==b2Joint.e_pulleyJoint)
		{
		...
		else if(joint.m_type==b2Joint.e_mouseJoint)
		{
			m_debugDraw.DrawSegment(p1, p2, color);
		}
		else
	...
	public void DrawShape(b2Shape shape, b2Transform xf, b2Color color){
		...
		else if(shape.m_type==b2Shape.e_polygonShape)
		{
			int i;
		...
		else if(shape.m_type==b2Shape.e_edgeShape)
		{
	...
	public b2ContactManager m_contactManager = new b2ContactManager();
	
	// These two are stored purely for efficiency purposes, they don't maintain
	// any data outside of a call to Step
	private b2ContactSolver m_contactSolver = new b2ContactSolver();
	private b2Island m_island = new b2Island();

	public b2Body m_bodyList;
	...
	private b2Controller m_controllerList;
	...
	static private bool m_warmStarting;

	// This is for debugging the solver.
	static private bool m_continuousPhysics;
	
	// m_flags
	public const int e_newFixture = 0x0001;
	public const int e_locked = 0x0002;
